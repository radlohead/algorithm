...
// 정문에서 엘리베이터까지의 거리는 무시한다. 또 모든 인접한 두 방 사이의 거리는 같은 거리(거리 1)라고 가정하고
...
const app = (roomCount, floorCount, guestCount) => {
    let count = 1
    while (roomCount < guestCount) {
        guestCount -= roomCount
        ++count
    }
    if (floorCount < count) return
    return guestCount * 100 + count
}
