...
const solution = baseball => {
    ...
    for (let i = 123; i <= 987; i++) {
        ...
        let z = Math.floor(i / 100)

        if (x === 0 || y === 0 || z === 0) continue
        if (x === y || y === z || x === z) continue

        for (let j = 0; j <= baseball.length - 1; j++) {
            let strike = 0
            let ball = 0

            let question = baseball[j][0]
            let question_x = question % 10
            let question_y = Math.floor(question / 10) % 10
            let question_z = Math.floor(question / 100)
