...
// 배열 speeds가 주어질 각 배포마다 몇 개의 기능이 배포되는지를 return 하도록 solution 함수를 완성하세요.
// 제한 사항
...
function solution(progress: number[], speed: number[]) {
    ...
    progress.map((v: number, i: number): void => {
        for(let j: number = speed[i], k: number = speed[i]; v+k <= 100; k += speed[i]){
            if (j+k+v >= 100) {
                arr.push((k / j) + 1);
                return;
            }
...
const progress: number[] = [93,30,55];
const speed: number[] = [1,30,5];
