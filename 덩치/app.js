...
// 우리는 A의 덩치가 B의 덩치보다 "더 크다"고 말한다. 예를 들어 어떤 A, B 두 사람의 덩치가 각각 (56,177), (45,165)
// 라고 한다면 A의 덩치가 B보다 큰 셈이 된다. 그런데 서로 다른 덩치끼리 크기를 정할 수 없는 경우도 있다.
...
const app = (n, nArr) => {
    const weightSortList = nArr.slice().sort((a, b) => b[0] - a[0])
    const tempObj = weightSortList.map(v => {
        return {
            list: v,
            count: 0,
            index: 0
    ...
    let lastCount = 0
    tempObj.map((v, i, arr) => {
        if (arr.length - 1 > i && v.list[1] > arr[i + 1].list[1]) {
            v.count = lastCount + 1
            lastCount = v.count
        }
        if (arr.length - 1 > i && v.list[1] < arr[i + 1].list[1]) {
        ...
        if (arr.length - 1 === i && arr[i - 1].list[1] > v.list[1]) {
            v.count = arr.length
        }
    })
    const tempObjList = tempObj.map(v => v.list)
    const tempSortIndex = index => {
        return nArr.findIndex((v, i) => {
            return v === tempObjList[index]
        })
    }
    const result = tempObj
        .map((v, i) => {
            v.index = tempSortIndex(i)
            return v
        })
...
app(5, [[55, 185], [58, 183], [88, 186], [60, 175], [46, 155]])
