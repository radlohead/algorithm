...
function solution(genres, plays) {
    ...
    genres.forEach(v => key.add(v));
    key.forEach(v => keyArr.push(v));
    genres.forEach(v => obj[v] = 0);
    genres.forEach((v, i) => obj[v] += plays[i]);
    for(o in obj) temp.push(obj[o]);
    temp.sort((a, b) => b - a);

    for(o in obj) {
        temp.findIndex((v, i) => {
            if (v === obj[o]) ranking[i] = o;
        });
    }
