...
function solution(arg: number[]): string {
    const arr: number[] = [];
    const depth: number = 0;
    const n: number = arg.length;

    function swap(arg: number[], i: number, j: number) {
        const temp: number = arg[i];
        arg[i] = arg[j];
        arg[j] = temp;
    }

    function func(arg: number[], depth: number, n: number) {
        if (depth === n) {
...
const numbers: number[] = [3, 30, 34, 5, 9];
solution(numbers);