...
const app = (n: number, arg: number[][]) => {
    const arr = arg.sort((a: number[], b: number[]) => a[0] - b[0])
    const firstDownNumberList = arr.filter(v => v[0] < v[1])
    const firstUpNumberList = arr.filter(v => v[0] > v[1])
    const sameNumberList = arr.filter(v => v[0] === v[1]).map(v => v[0])
    const result = [firstDownNumberList.length, firstUpNumberList.length]

    firstDownNumberList.forEach((v: number[], i: number) => {
        if (sameNumberList[i] >= v[0] && sameNumberList[i] <= v[1]) {
            ++result[0]
        }
    })
    firstUpNumberList.forEach((v: number[], i: number) => {
        if (sameNumberList[i] >= v[0] && sameNumberList[i] <= v[1]) {
            ++result[1]
        }
    })
...
const arr = [[1, 8], [3, 9], [2, 2], [4, 1], [6, 4], [10, 10], [9, 7], [7, 6]]
app(8, arr)
