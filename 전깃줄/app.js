...
// 예제 출력 1
// 3
var app = function (n, arg) {
    var arr = arg.sort(function (a, b) { return a[0] - b[0]; });
    var firstDownNumberList = arr.filter(function (v) { return v[0] < v[1]; });
    var firstUpNumberList = arr.filter(function (v) { return v[0] > v[1]; });
    var sameNumberList = arr.filter(function (v) { return v[0] === v[1]; }).map(function (v) { return v[0]; });
    var result = [firstDownNumberList.length, firstUpNumberList.length];
    firstDownNumberList.forEach(function (v, i) {
        if (sameNumberList[i] >= v[0] && sameNumberList[i] <= v[1]) {
            ++result[0];
        }
    });
    firstUpNumberList.forEach(function (v, i) {
        if (sameNumberList[i] >= v[0] && sameNumberList[i] <= v[1]) {
            ++result[1];
        }
    });
...
var arr = [[1, 8], [3, 9], [2, 2], [4, 1], [6, 4], [10, 10], [9, 7], [7, 6]];
app(8, arr);
