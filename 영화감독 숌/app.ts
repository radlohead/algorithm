...
// 스타워즈 6과 같이 이름을 지었고, 피터 잭슨은 반지의 제왕을 만들 때, 반지의 제왕 1,
// 반지의 제왕 2, 반지의 제왕 3과 같이 영화 제목을 지었다.
...
const app = (n: number): number => {
    const arr: number[] = Array(1).fill(n - 1)
    const featureCount = 666

    arr.push(featureCount)
    return arr[0] <= 0 ? featureCount : Number(arr.join().replace(',', ''))
}
app(0) // 666
app(1) // 666
app(2) // 1666
